...
fn test_works() {
  #[track_caller]
  fn test_json_import(input: &str, expected: &str) {
    assert_eq!(
      to_string(&from_str::<Value>(input).unwrap()).unwrap(),
      expected
    );
  }

  test_json_import(
    include_str!("fixtures/ex.1.input.json").trim(),
    include_str!("fixtures/ex.1.output.json").trim(),
  );

  test_json_import(
    include_str!("fixtures/ex.2.input.json").trim(),
    include_str!("fixtures/ex.2.output.json").trim(),
  );

  test_json_import(
    include_str!("fixtures/ex.3.input.json").trim(),
    include_str!("fixtures/ex.3.output.json").trim(),
  );
